        ...
        return (
            <Table>
                ...
                <TableBody displayRowCheckbox={false}>
                    {users.map(userJson => {
                        let user = new User(users.request);
                        user.setAttributes(userJson);

                        return (
                            <ManageUsersTableRow key={user.id} user={user} users={users} currentUser={currentUser}/>
                        );
                    })}
                    <ManageUsersTableRow key="new" user={new User(users.request)} users={users} currentUser={currentUser}/>
                </TableBody>
            </Table>
        );
    }
